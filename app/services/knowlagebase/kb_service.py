...
class KnowledgeBaseService:
    ...
    def _get_storage(self) -> QdrantStorage:
        if self._storage is None:
            self._storage = QdrantStorage(qdrant_db.client, embeddings)
    ...
    async def create_knowledgebase(self, base_name: str, file_content: bytes) -> Dict:
        ...
        await storage.init_collections([parent_col, child_col])
        ...
        await storage.store_parent_section(
        ...
        await storage.store_batang_tubuh(parent_col, child_col, doc_structure["pasal_list"], doc_id, doc_structure["metadata"])
        
        if doc_structure.get("penjelasan"):
            await storage.store_penjelasan(parent_col, child_col, doc_structure["penjelasan"], doc_id, doc_structure["metadata"])
    ...
    async def get_collection_stats(self, base_name: str) -> Dict:
        return await self._get_storage().get_stats(base_name)
    
    async def get_kb_info(self, base_name: str) -> Dict:
        return await self._get_storage().get_kb_info(base_name)
    ...
    async def search_knowledgebase(
        ...
        return await self._get_storage().search_knowledgebase(
